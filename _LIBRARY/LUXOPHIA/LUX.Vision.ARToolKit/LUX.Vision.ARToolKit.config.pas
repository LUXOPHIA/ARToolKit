...
type T_AR_PIXEL_FORMAT = (
     ...
     P_AR_PIXEL_FORMAT = ^T_AR_PIXEL_FORMAT;
...
(*
  {$IF ( __BYTE_ORDER__ = __ORDER_BIG_ENDIAN__ ) or defined( __BIG_ENDIAN__ ) }
    ...
    {$UNDEF AR_LITTLE_ENDIAN }
  {$ELSEIF ( __BYTE_ORDER__ = __ORDER_LITTLE_ENDIAN__ ) or defined( __LITTLE_ENDIAN__ ) }
...
// Endianness.
  {$IF TARGET_RT_BIG_ENDIAN }
    {$DEFINE AR_BIG_ENDIAN }  // Most Significant Byte has greatest address in memory (ppc).
    {$UNDEF AR_LITTLE_ENDIAN }
  {$ELSEIF TARGET_RT_LITTLE_ENDIAN }
    {$UNDEF AR_BIG_ENDIAN }
    {$DEFINE AR_LITTLE_ENDIAN }
  {$ELSE}
    //#error
